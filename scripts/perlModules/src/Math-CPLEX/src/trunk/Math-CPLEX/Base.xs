...
###############################################################################
int
_presolve(CPXENVptr g_cplex_env, CPXLPptr g_cplex_lp, int method)
   PREINIT:
      int status;
   CODE:
      status = CPXpresolve(g_cplex_env, g_cplex_lp, method);

      if( status )
      {
         fprintf(stderr, "ERROR: CPXpresolve() failed. Returned status: %d\n",status);
         print_CPLEX_error_string(g_cplex_env, status);
         XSRETURN_UNDEF;
      }
      else
...
###############################################################################
int
_readcopyprob(CPXENVptr g_cplex_env, CPXLPptr g_cplex_lp, char *filename, char *format)
   PREINIT:
      int status;
   CODE:
      status = CPXreadcopyprob(g_cplex_env, g_cplex_lp, filename, format);
      if( status )
      {
        fprintf(stderr, "ERROR: CPXreadcopyprob() from file '%s' in format '%s' failed. Returned status: %d\n", filename, format, status);
        print_CPLEX_error_string(g_cplex_env, status);
        XSRETURN_UNDEF;
      }
      else
...
###############################################################################
int
_getrowindex(CPXENVptr g_cplex_env, CPXLPptr g_cplex_lp, char* rowname)
   PREINIT:
      int row_index;
      int status;
   CODE:
      status = CPXgetrowindex(g_cplex_env, g_cplex_lp, rowname, &row_index);
      if( status )
      {
        fprintf(stderr, "ERROR: CPXgetrowindex('%s') failed. Returned status: %d\n", rowname, status);
        print_CPLEX_error_string(g_cplex_env, status);
        XSRETURN_UNDEF;
      }
      else
...
###############################################################################
int
_getcolindex(CPXENVptr g_cplex_env, CPXLPptr g_cplex_lp, char* colname)
   PREINIT:
      int col_index;
      int status;
   CODE:
      status = CPXgetcolindex(g_cplex_env, g_cplex_lp, colname, &col_index);
      if( status )
      {
        fprintf(stderr, "ERROR: CPXgetcolindex('%s') failed. Returned status: %d\n", colname, status);
        print_CPLEX_error_string(g_cplex_env, status);
        XSRETURN_UNDEF;
      }
      else
