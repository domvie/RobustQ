...
sub minimize
{
   ...
   {
      carp "$whoami: Couldn't get CPLEX environment.";
      return undef;
   }
...
sub presolve
{
   ...
   croak "$whoami: presolve method was not provided" unless defined $method;

   Math::CPLEX::Base::_presolve($env, $self->{lp}, $method);
}
###############################################################################
...
sub readcopyprob
{
   ...
   Math::CPLEX::Base::_readcopyprob($env, $self->{lp}, $filename, $format);
}
###############################################################################
...
sub getcolindex
{
   ...
   Math::CPLEX::Base::_getcolindex($env, $self->{lp}, $colname);
}
###############################################################################
...
sub getrowindex
{
   ...
   Math::CPLEX::Base::_getrowindex($env, $self->{lp}, $rowname);
}
###############################################################################
...
In case of an error, undef is returned.
   ...
   my $col_idx = $lp->getcolindex($column_name);

=head2 getrowindex

Get the index of a row by row name.
In case of an error, undef is returned.

   my $row_idx = $lp->getrowindex($row_name);
...
Execute the CPLEX presolve routine solver. The method returns C<undef> if not successful.
Possible values are CPX_ALG_NONE, CPX_ALG_PRIMAL, CPX_ALG_DUAL, and CPX_ALG_BARRIER for LP; CPX_ALG_NONE should be used for MIP.
   ...
   $method = &Math::CPLEX::Base::CPX_ALG_NONE;
   $lp->presolve($method);
...
Read a optimization problem form file and copy it to an existing CPLEX problem.
The first parameter specifies the name of the file that is written.
The second parameter specifies the file format. The second parameter
is optional and by default the file format 'LP' is used.
If reading or copying fails, undef returned.

   $lp->readcopyprob($filename);
   $lp->readcopyprob($filename, 'MPS');
...
Matthias Gerstl, E<lt>matthias.gerstl@boku.ac.atE<gt>

=head1 COPYRIGHT AND LICENSE

Copyright (C) 2013,2014,2015 by Christian Jungreuthmayer and Matthias Gerstl
