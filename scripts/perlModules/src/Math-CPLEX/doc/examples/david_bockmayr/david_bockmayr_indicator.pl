...
my $num_tgts = 0;
my ($cplex_env_sub,$lp_sub);
my ($cplex_env_efm,$lp_efm);
my $num_reacs_expand;
...
sub do_david_bockmayr
{
   ...
   $cplex_env_sub = Math::CPLEX::Env->new();
   die "ERROR: creating CPLEX environment failed!" unless $cplex_env_sub;

   # create MIP problem
   $lp_sub = $cplex_env_sub->createOP();
   die "ERROR: couldn't create Linear Program\n" unless $lp_sub;

   die "ERROR: minimize() failed\n" unless $lp_sub->minimize();

   print "INFO: going to use $num_threads parallel threads\n";
   $cplex_env_sub->setintparam(&Math::CPLEX::Base::CPX_PARAM_THREADS, $num_threads);
   $cplex_env_sub->setintparam(&Math::CPLEX::Base::CPX_PARAM_SCRIND, &Math::CPLEX::Base::CPX_ON);

   fill_init_lp_network();

   while(1)
   {
      my $subnet = find_subnet() or last;
      compute_efm_for_subnet($subnet);
   }

   #############################################################################
   #############################################################################
   die "ERROR: free() failed\n" unless $lp_sub->free();
   die "ERROR: close() failed\n" unless $cplex_env_sub->close();
   #############################################################################
}
################################################################################
...
################################################################################
################################################################################
sub compute_efm_for_subnet
{
   my $subnet = shift;

   # get CPLEX environment
   $cplex_env_efm = Math::CPLEX::Env->new();
   die "ERROR: creating CPLEX environment failed!" unless $cplex_env_efm;

   # create MIP problem
   $lp_efm = $cplex_env_efm->createOP();
   die "ERROR: couldn't create Linear Program\n" unless $lp_efm;

   die "ERROR: minimize() failed\n" unless $lp_efm->minimize();

   print "INFO: going to use $num_threads parallel threads\n";
   $cplex_env_efm->setintparam(&Math::CPLEX::Base::CPX_PARAM_THREADS, $num_threads);
   $cplex_env_efm->setintparam(&Math::CPLEX::Base::CPX_PARAM_SCRIND, &Math::CPLEX::Base::CPX_ON);

   fill_init_lp_efm($subnet);

   #############################################################################
   # solve MIP problem
   #############################################################################
   die "ERROR: mipopt() failed\n" unless $lp_efm->mipopt();
   #############################################################################
   ...
   #############################################################################
   if( $lp_efm->getstat() != &Math::CPLEX::Base::CPXMIP_OPTIMAL )
   {
      print "CPLEX didn't find an optimal solution (status=", $lp_efm->getstat(), ") -> algorithm stopped.\n";
   ...
   my ($sol_status, $obj_val, @vals) = $lp_efm->solution();
   print "compute_efm_for_subnet: sol_status=$sol_status, obj_val=$obj_val\n";
   print "                        vals=@vals\n";
   ...
   # add solution to linear program for subnetwork
   add_solution();
   
   $solution_cnt++;

   die "ERROR: free() failed\n" unless $lp_efm->free();
   die "ERROR: close() failed\n" unless $cplex_env_efm->close();
}
################################################################################

################################################################################
################################################################################
sub fill_init_lp_efm
{
   my $subnet = shift;
   ...
   if( $write_lp_file )
   {
      my $filename = "/tmp/myCPLEX_efm_" . $solution_cnt . ".lp";
      print "INFO: going to write lp-file '$filename'\n";
      die "ERROR: writeprob() failed\n" unless $lp_efm->writeprob($filename);
   }
   #############################################################################
}
################################################################################
...
sub add_rows_efm
{
   ...
   for( my $m = 0; $m < @$metas; $m++ )
   {
      ...
      for( my $r = 0; $r < $num_reacs_expand; $r++ )
      {
         ...
         $newRows->[$row_cnt][$r] = $subnet->[$m][$r] if abs($subnet->[$m][$r]) > CONSIDER_ZERO;
      }
      $row_cnt++;
   }
   ...
   if( $num_tgts > 0 )
   {
      $rhs->[$row_cnt]                     = 1.0;
      $sense->[$row_cnt]                   = 'G';
      $row_names->[$row_cnt]               = "vr_$target_idx->[0]";
      $newRows->[$row_cnt][$target_idx->[0]] = 1.0;
      $row_cnt++;
   }

   # testing Bockmayr approach
   #$rhs->[$row_cnt]                     = 1.0;
   #$sense->[$row_cnt]                   = 'G';
   #$row_names->[$row_cnt]               = "vr_R7";
   #$newRows->[$row_cnt][6] = 1.0;
   #$row_cnt++;
...
sub find_subnet
{
   ...
   # retrieve computed values
   #############################################################################
   if( $lp_sub->getstat() != &Math::CPLEX::Base::CPXMIP_OPTIMAL )
   {
      print "CPLEX didn't find an optimal solution (status=", $lp_sub->getstat(), ") -> algorithm stopped.\n";
      print "return value if optimal solution was obtained: ", &Math::CPLEX::Base::CPXMIP_OPTIMAL, "\n";
      print "number of computed elementary flux modes: $solution_cnt\n";
      print "elementary flux modes were written to '$efm_filename'\n";
      return 0;
   ...
   my ($sol_status, $obj_val, @vals) = $lp_sub->solution();
...
sub add_solution
{
   ...
   if( $write_lp_file )
   {
      ...
      my $filename = "/tmp/myCPLEX_network_$file_num.lp";
      print "INFO: going to write lp-file '$filename'\n";
      die "ERROR: writeprob() failed\n" unless $lp_sub->writeprob($filename);
   }
   #############################################################################
}
################################################################################
...
sub fill_init_lp_network
{
   ...
   die "ERROR: newcols() failed\n" unless $lp_sub->newcols($cols);
   #############################################################################

   #############################################################################
   # add initial rows
   #############################################################################
   add_inital_rows_network();
   #############################################################################
   ...
   if( $write_lp_file )
   {
      my $filename = "/tmp/myCPLEX_network.lp";
      print "INFO: going to write lp-file '$filename'\n";
      die "ERROR: writeprob() failed\n" unless $lp_sub->writeprob($filename);
   }
   #############################################################################
...
sub add_inital_rows_network
{
   ...
   for( my $i = 0; $i < $num_reacs_expand; $i++ )
   {
      ...
      die "ERROR: addindconstr() failed\n" unless $lp_sub->addindconstr($indconstr);
   }
   ...
   for( my $t = 0; $t < $num_tgts - 1; $t++ )
   {
      ...
      for( my $r = 0; $r < @$reacs_exp; $r++ )
      {
         ...
         else
         {
            ...
            for( my $m = 0; $m < @$metas; $m++ )
            {
               $val->[2*$num_reacs_expand + $t*scalar(@$metas) + $m] = $stoim_exp_T->[$r][$m];
            }
            ...
            if( $r == $target_idx->[$t] )
            {
               $val->[2*$num_reacs_expand + @$metas*($num_tgts-1) + $t] = 1.0;
            }


            my $indconstr = {
                               indvar       => $num_reacs_expand + $r,
                               complemented => 0,
                               rhs          => 0.0,
                               sense        => 'G',
                               val          => $val,
                               name         => "IndicaST_Dirc_" . $target_name->[$t] . "_" . $target_name->[$t+1],
                            };
...
sub read_targets
{
   ...
   for( my $i = 0; $i < @$tgts; $i++ )
   {
      die "ERROR: target reaction $reacs->[$i] is reversible -> not supported yet\n" if( $tgts->[$i] != 0 && $rv->[$i] != 0 );
...
sub create_subnet
{
   ...
   for( my $m = 0; $m < @$stoim_exp; $m++ )
   {
      for( my $r = 0; $r < @{$stoim_exp->[$m]}; $r++ )
      {
         if( $cur_as[$r] > 0.5 )
         {
            $subnet->[$m][$r] = $stoim_exp->[$m][$r];
         }
         else
         {
            $subnet->[$m][$r] = 0;
...
sub usage
{
   ...
   print "david_bockmayr.pl -s sfile -m file -r rfile -v rvfile -t tffile -o modes_filename [-h -l -p num_threads -k max_efms]\n";
   print "\n";
   ...
   print "\n";
   print "david_bockmayr.pl computes elementary flux modes of a given metabolic system\n";
